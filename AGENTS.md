...
- Images referenced by absolute path live in `public/` (e.g. `/screens/*.png`,
  `/skins/dracula.png`). The Starlight logo / hero image lives in `src/assets/`.
...
- `.github/workflows/deploy.yml` builds with the official `withastro/action@v6`
  and deploys via `actions/deploy-pages` on pushes to `master`.
